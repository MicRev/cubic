...
const char lightness[6] = {'.', '-', 's', 'K', '#', '@'};

const int D = 100;
const double CERTAINTY = 1e-6;
Eigen::Vector3d light(0., -0.5, -0.86602540378443864676372317075294); // (0, -1/2, -\sqrt{3}/2)
...
Cubic::Cubic() {
    ...
    get_light_order();
    _view_norm_vec << -1, 0, 0;
}

Cubic::Cubic(unsigned int theta, unsigned int phi) : Cubic() {
...
char Cubic::light_of(Eigen::Vector2d coord) {
    ...
    Eigen::Vector3d rot_view_norm_vec = inv_rot_mat * _view_norm_vec; 
    ...
    double nx = rot_view_norm_vec(0);
    double ny = rot_view_norm_vec(1);
    double nz = rot_view_norm_vec(2);
    ...
    base_mat <<  ny / norm_coord_x, -nx*nz / norm_coord_y, 
                -nx / norm_coord_x, -ny*nz / norm_coord_y, 
                    0.               , (nx*nx+ny*ny) / norm_coord_y; // col 1: \vec{b}_1 = \vec{n} \times \hat{k}; col 2: \vec{b}_2 = \vec{b}_1 \times \vec{n}, then normalize
    ...
    int size = 0;
    Eigen::Vector3d points_on_surface[3];
    for (int i = 0; i < 6; ++i) {
        bool is_valid_point = true;
        int idx = i % 3;  // x, y, or z
        double val = double(i / 3) - 0.5;
        Eigen::Vector3d p;
        p(idx) = val;
        ...
        if (is_valid_point) {
            points_on_surface[size++] = p;
        }

        if (size > 2) {
            std::cout <<  "Too many points found on surface! Wrong algorithm!" << std::endl;
            throw "Too many points found on surface! Wrong algorithm!";
        }
    }
    ...
    if (size != 2) {
        std::cout << "Only" << size << " points found on surface!" << std::endl;
        throw "Only %d points found on surface!", size;
    }

    double n1, n2;
    n1 = (unrot_vec - points_on_surface[0]).norm();
    n2 = (unrot_vec - points_on_surface[1]).norm();

    Eigen::Vector3d targ_point = n1 < n2 ? points_on_surface[0] : points_on_surface[1];
    int surface = which_surface(targ_point);
